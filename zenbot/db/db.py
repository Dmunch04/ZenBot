...
class Database:
    def __init__(self):
        ...
        if self.use_auth:
            ...
            self.uri: str = (
                f"mongodb://{self.host}:{self.port}/?authSource={self.name}&ssl=false"
            )
        ...
        self.server_collection: AsyncIOMotorCollection = self.db.get_collection(
            self.collection
        )
    ...
    async def find_by_id(self, id: str) -> Dict[str, Any]:
        ...
        cursor: AsyncIOMotorCursor = self.server_collection.find(
